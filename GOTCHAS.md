...
Added `window.appLocale()` (`'es-ES'` when `window.currentLang === 'es'`, else `'en-US'`) — pass this instead of `undefined` anywhere a date/time gets formatted for display. **Only fixed the two sites the reported bug pointed at.** A repo-wide `grep -c "toLocaleDateString(undefined\|toLocaleTimeString(undefined"` turns up **36 occurrences** — this is very likely a widespread pattern across event cards, dashboard tables, reports, and more, not just these two. Worth a deliberate full sweep at some point; scoped out of this fix since it wasn't what was reported and 36 sites is a real chunk of work to verify individually.
