...
- **`#app` and `#give` removed from `validRoutes`.** `#page-app` is a marketing page for a **mobile app that does not exist** ("Download on the App Store" / "Get it on Google Play" — non-functional `<div>`s); `#page-give` is a **dead** standalone donation page (the real giving flow is a tab on the church page — nothing ever routes to `#give`). Both were reachable only by typing the URL and both render the same unlabeled "Grace Fellowship Church" sample content. Now they resolve to `#home` via the existing `validRoutes.indexOf(base) === -1` fallback. The dead `#page-app` / `#page-give` HTML sections are left in place (unreachable, harmless). `nav.app` i18n key deleted (EN + ES); **`conference` and `compare` left untouched** per the "separate work" carve-out.
