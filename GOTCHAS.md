...
**Total: 9 functions fixed** across `get_pending_church_claims`, `review_church_claim`, `get_churches_without_coordinates`, `search_users_admin`, `admin_delete_unclaimed_church`, `get_church_claims_history`, `search_all_churches_admin`, `admin_import_churches`, `get_group_notification_recipients`. Minor inconsistency worth knowing about: the first two fixes inline the `profiles.is_platform_admin` check directly rather than calling the existing `is_platform_admin()` helper function (only discovered to exist once the full function list came back) — functionally identical, but worth normalizing to the helper for consistency if anyone touches those two again.
