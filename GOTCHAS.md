...
**The actual fix: widen the real container instead of fighting Turnstile's requirement.** `.auth-wrap`'s own padding dropped from 28px to 16px, and `#auth-turnstile-wrap` specifically breaks out of `.auth-card`'s 36px padding with negative margins — every other element in the card keeps its original spacing; only the Turnstile row uses the wrap's full width. This pushes real content width comfortably past 300px on ordinary phone widths (~360px+), so `data-size="flexible"` now has the room it always needed, and renders at its native 65px height instead of compact's 140px.
